...
class PluginsManager:
    ...
    def _add_assistant_entry(self):
      """Register a 'SALOME Assistant' menu entry when the SALOME_ASSISTANT
      environment variable is defined.

      SALOME_ASSISTANT must point to the salome-assistant executable installed
      in its own isolated virtual environment, e.g.:
          export SALOME_ASSISTANT=/opt/salome-assistant-venv/bin/salome-assistant
      """
      assistant_exe = os.environ.get('SALOME_ASSISTANT')
      if not assistant_exe:
        return

      def launch_assistant(context, exe=assistant_exe):
        import subprocess
        try:
          # Build a clean environment for the assistant's isolated venv.
          # SALOME sets PYTHONPATH, LD_LIBRARY_PATH and several Qt variables
          # that redirect Python imports and Qt plugin loading to SALOME's own
          # libraries, which are incompatible with the pip-installed PyQt5
          # bundled inside the venv.  Stripping them lets the venv's Python
          # and PyQt5 use their own self-contained libraries.
          env = os.environ.copy()
          for var in (
            'PYTHONPATH',
            'QT_PLUGIN_PATH',
      ...
      self.AddFunction(
        'SALOME Assistant',
        'Launch the SALOME AI Assistant (RAG-based)',
        launch_assistant
      )
    ...
    def importPlugins(self):
        ...
        if not plugins_files:
          ...
          self.lasttime=0
          self.menu.clear()
          # Even without plugin files, show the menu if SALOME_ASSISTANT is set.
          if os.environ.get('SALOME_ASSISTANT'):
            self._add_assistant_entry()
            self.updateMenu()
          else:
            self.menu.menuAction().setVisible(False)
          return
        ...
        if self.plugins_files != plugins_files or lasttime > self.lasttime:
          ...
          # Append SALOME Assistant entry at the end of the plugins list
          # when the SALOME_ASSISTANT environment variable is defined.
          self._add_assistant_entry()
          self.updateMenu()
